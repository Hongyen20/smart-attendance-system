...
// Màn hình "Duyệt đơn" của Admin - gộp 3 loại đơn cần duyệt vào 3 tab con:
// Nghỉ phép, Đổi ca làm việc, và Công tác.
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: const TabBarView(
          children: [
            _LeaveApprovalTab(),
            _ShiftChangeApprovalTab(),
            _BusinessTripApprovalTab(),
          ],
        ),
      ),
    );
...
class _LeaveApprovalTab extends StatefulWidget {
  const _LeaveApprovalTab();

  @override
  State<_LeaveApprovalTab> createState() => _LeaveApprovalTabState();
}

class _LeaveApprovalTabState extends State<_LeaveApprovalTab> {
  ...
  Future<void> _loadPending() async {
    ...
    final result = await ApiService.getList('/api/leave-requests/pending', bearerToken: AuthState.instance.token);
  ...
  Future<void> _handleDecision(String id, bool approve) async {
    ...
    if (result.success) {
      ...
      if (approve) {
        ...
        message = isPaid
            ? 'Đã duyệt đơn - Nghỉ phép có lương (đã trừ hạn mức).'
            : 'Đã duyệt đơn - Hết hạn mức, xử lý nghỉ không lương.';
  ...
  @override
  Widget build(BuildContext context) {
    return RefreshIndicator(onRefresh: _loadPending, child: _buildBody());
  }
  ...
  Widget _buildBody() {
    ...
    if (_pendingRequests.isEmpty) {
      return ListView(
        ...
        children: const [
          SizedBox(height: 60),
          Icon(Icons.fact_check_outlined, color: AppColors.textSecondary, size: 48),
          SizedBox(height: 12),
          Center(child: Text('Không có đơn nào đang chờ duyệt.', style: TextStyle(color: AppColors.textSecondary))),
        ],
      );
    }
        ...
        children: [
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(r['type'] ?? '', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.pendingBlue)),
              ),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              ...
              Text(_formatDateRange(r), style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
            ],
          ),
...
// ============================================================
// Tab: Duyệt yêu cầu đổi ca
// ============================================================

class _ShiftChangeApprovalTab extends StatefulWidget {
  const _ShiftChangeApprovalTab();

  @override
  State<_ShiftChangeApprovalTab> createState() => _ShiftChangeApprovalTabState();
}

class _ShiftChangeApprovalTabState extends State<_ShiftChangeApprovalTab> {
  bool _isLoading = true;
  String? _errorMessage;
  List<dynamic> _pendingRequests = [];
  final Set<String> _processingIds = {};
...
// ============================================================
// Tab: Duyệt yêu cầu đi công tác
// ============================================================

class _BusinessTripApprovalTab extends StatefulWidget {
  const _BusinessTripApprovalTab();

  @override
  State<_BusinessTripApprovalTab> createState() => _BusinessTripApprovalTabState();
}

class _BusinessTripApprovalTabState extends State<_BusinessTripApprovalTab> {
  bool _isLoading = true;
  String? _errorMessage;
  List<dynamic> _pendingRequests = [];
  final Set<String> _processingIds = {};
  ...
  Future<void> _loadPending() async {
    ...
    final result = await ApiService.getList('/api/business-trip-requests/pending', bearerToken: AuthState.instance.token);
  ...
  Future<void> _handleDecision(String id, bool approve) async {
    ...
    final result = await ApiService.put(
      '/api/business-trip-requests/$id/${approve ? 'approve' : 'reject'}',
      {},
      bearerToken: AuthState.instance.token,
    );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(approve
              ? 'Đã duyệt - các ngày công tác sẽ bỏ qua xác thực IP/GPS khi check-in.'
              : 'Đã từ chối yêu cầu công tác.'),
          backgroundColor: approve ? AppColors.successGreen : AppColors.dangerRed,
        ),
      );
    } else {
  ...
  Widget _buildBody() {
    ...
    if (_pendingRequests.isEmpty) {
      return ListView(
        ...
        children: const [
          SizedBox(height: 60),
          Icon(Icons.card_travel_outlined, color: AppColors.textSecondary, size: 48),
          SizedBox(height: 12),
          Center(child: Text('Không có yêu cầu nào đang chờ duyệt.', style: TextStyle(color: AppColors.textSecondary))),
        ],
      );
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(r['employeeName'] ?? 'Không rõ',
                        style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                    Text(r['employeeUsername'] ?? '', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                  ],
                ),
              ),
            ],
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              Expanded(
                child: Text(r['destination'] ?? '', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
              ),
            ],
          ),
          const SizedBox(height: 6),
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              Text(
                '${_formatDate(r['startDate'] as String)} - ${_formatDate(r['endDate'] as String)}',
                style: const TextStyle(fontSize: 13, color: AppColors.textSecondary),
              ),
            ],
          ),
